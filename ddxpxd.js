...
叮咚下单后自动领取
[task_local]
55 23 * * * https://raw.githubusercontent.com/justplayscript/ddxp/main/ddxpxd.js, tag=叮咚下单后自动领取, enabled=true
*/

const $ = new Env('叮咚下单后自动领取');
const dr = "@"
let ddxpurlArr = [],
    ddxphdArr = [],
    ddxpcount = ''
...
//获取果园所有任务
function ddxpgyTaskLog(timeout = 0) {
    userTaskLogId = ""
    return new Promise((resolve) => {
        let header = pubHeader()
        ...
        let url = {
            url: `https://farm.api.ddxq.mobi/api/v2/task/list-orchard?api_version=9.1.0&app_client_id=1&station_id=${station_id}&native_version=&uid=${uid}&latitude=${latitude}&longitude=${longitude}&reward=FERTILIZER&cityCode=1103`,
            headers: header,
        }
